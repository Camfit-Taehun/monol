...
export class DailyScan {
     ...
     * Run discovery phase
     */
    async runDiscoveryPhase(result) {
        try {
            ...
            const topCandidates = discoveryResult.candidates.slice(0, this.config.maxNewCandidates);
     ...
     * Run evolution phase
     */
    async runEvolutionPhase(result) {
        try {
            ...
            result.errors.push(`Evolution failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     * Run internalization phase
     */
    async runInternalizationPhase(result) {
        try {
            ...
            result.errors.push(`Internalization failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     * Start trials for new candidates
     */
    async startTrialsForCandidates(result) {
...
export function createDailyScan(discovery, evolution, internalization, config) {
    return new DailyScan(discovery, evolution, internalization, config);
}
//# sourceMappingURL=daily-scan.js.map