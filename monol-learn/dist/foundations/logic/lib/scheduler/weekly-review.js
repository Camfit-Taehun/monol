...
export class WeeklyReview {
    ...
    lastRun;
    results = [];
    constructor(evolution, internalization, config = {}) {
    ...
    async runCleanupPhase(result) {
        try {
            ...
            for (const skill of lowPerformers) {
                this.evolution.promoter.archive(skill.id, `Archived due to low performance (score: ${skill.compositeScore.toFixed(1)})`);
                result.cleanup.lowPerformersArchived++;
                // Create lesson from archived skill
                this.internalization.internalizeArchivedSkill(skill, 'Low performance in weekly review');
            }
            // Remove duplicates
    ...
    generateRecommendations(result) {
        ...
        if (summary.byStatus.candidate > 10) {
            result.recommendations.push(`Consider starting more trials - ${summary.byStatus.candidate} candidates waiting`);
        }
        if (summary.runningTrials > 20) {
            result.recommendations.push(`Many trials running (${summary.runningTrials}) - consider focusing on completion`);
        }
        if (result.analysis.underperformingSkills.length > 3) {
            result.recommendations.push(`Review underperforming skills - ${result.analysis.underperformingSkills.length} skills near archive threshold`);
        }
        if (summary.byStatus.active < 5) {
            result.recommendations.push('Active skill count is low - consider promoting high-confidence candidates');
        }
        if (result.cleanup.staleTrialsCancelled > 5) {
            result.recommendations.push('Many stale trials found - review trial completion process');
        }
    }
    /**
     * Find stale trials
    ...
    findLowPerformers() {
        ...
        return this.evolution.candidates
            .getByStatus('active')
            .filter((s) => s.compositeScore < threshold &&
     ...
     */
    createPerformanceSummary(skill) {
        return {
            id: skill.id,
            name: skill.name,
    ...
    generateReport(result) {
        ...
        for (const skill of result.analysis.topPerformingSkills) {
            lines.push(`- **${skill.name}**: ${skill.compositeScore.toFixed(1)} (${skill.trend})`);
        }
        lines.push('');
...
export function createWeeklyReview(evolution, internalization, config) {
