...
export class RelevanceAnalyzer {
  ...
  analyze(candidate: SkillCandidate): RelevanceScore {
    ...
    // Weighted average
    const overall =
      projectMatch * 0.35 + novelty * 0.25 + utility * 0.25 + quality * 0.15;
  ...
  rankByRelevance(candidates: SkillCandidate[]): SkillCandidate[] {
    return [...candidates].sort(
      (a, b) => this.analyze(b).overall - this.analyze(a).overall
    );
  }

  private calculateProjectMatch(candidate: SkillCandidate): number {
    let score = 50; // Base score

    // Check language/framework match
    if (this.context.languages?.length) {
      const langMatch = this.matchTerms(
  ...
  private calculateNovelty(candidate: SkillCandidate): number {
    if (!this.context.existingSkills?.length) {
      return 70; // Default novelty score
    }

    // Check if similar skill exists
    const existingNames = this.context.existingSkills.map((s) =>
      s.name.toLowerCase()
    );
    ...
    const similarity = this.calculateSimilarity(
      candidate,
      this.context.existingSkills
    );
    return Math.max(0, 100 - similarity);
  }

  private calculateUtility(candidate: SkillCandidate): number {
    let score = 50;
  ...
  private estimateQuality(candidate: SkillCandidate): number {
  ...
  private matchTerms(text: string, terms: string[]): number {
  ...
  private calculateSimilarity(
    ...
    for (const skill of existingSkills) {
      ...
      const similarity =
        (intersection.length * 2) / (candidateWords.size + skillWords.size);
  ...
  private generateReasoning(
    candidate: SkillCandidate,
    scores: { projectMatch: number; novelty: number; utility: number; quality: number }
  ): string {
    const parts: string[] = [];
...
export function createRelevanceAnalyzer(
