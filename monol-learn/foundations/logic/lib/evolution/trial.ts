 ...
 */
export function createTrial(input: TrialCreationInput): EvolutionTrial {
  const now = new Date();
  return {
    id: generateTrialId(),
...
export class TrialManager {
  private trials: Map<string, EvolutionTrial> = new Map();
  ...
  recordBaseline(trialId: string, input: TrialRecordInput): EvolutionTrial | undefined {
    ...
    if (!trial || trial.status !== 'running') return undefined;
  ...
  recordPair(
    trialId: string,
    baseline: TrialRecordInput,
    challenger: TrialRecordInput
  ): EvolutionTrial | undefined {
    const trial = this.trials.get(trialId);
    if (!trial || trial.status !== 'running') return undefined;
   ...
   */
  private checkTrialProgress(trial: EvolutionTrial): void {
    // Update current trials count
    trial.currentTrials = Math.min(
      trial.baselineScores.length,
      trial.challengerScores.length
    );
   ...
   */
  private evaluateAndComplete(trial: EvolutionTrial): void {
    this.updateRecommendation(trial);
   ...
   */
  private updateRecommendation(trial: EvolutionTrial): void {
